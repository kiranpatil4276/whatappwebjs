...
const app = express();
const PORT = process.env.PORT || 3000; // Use PORT from environment for Render
...
app.get('/', (req, res) => {
  res.send('WhatsApp WebJS API is running. Use /qr to get QR and /send-file to send files.');
});

let qrCodeImage = '';
let isReady = false;

// Create WhatsApp client with safer Puppeteer args
const client = new Client({
  authStrategy: new LocalAuth(),
  puppeteer: {
    headless: true,
...
client.on('disconnected', (reason) => {
  ...
  client.initialize(); // Try to reinitialize
});

client.initialize();

// Endpoint to serve QR
app.get('/qr', (req, res) => {
  if (isReady) {
    return res.send({ status: 'authenticated' });
  } else if (qrCodeImage) {
